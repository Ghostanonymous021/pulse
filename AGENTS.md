...
- [x] Multiplas contas por numero de telefone + incidente de migracao sem trilha (25/07/2026):
  - **Incidente registado**: em 2026-07-25 06:14 UTC, `profiles_phone_key` (unique index) foi removido e `handle_new_user` foi alterado directamente na base de producao, sem ficheiro de migration versionado (viola a norma deste documento §3: "nunca alteracao manual direta em producao"). O limite ficou em 5 (nao 6), sem lock de concorrencia (`count` + `insert` nao atomico — corrida possivel sob signups simultaneos), e `find_accounts_by_phone` (funcao de lookup por numero) ficou com `EXECUTE` concedido a `anon` — permitia enumerar contas (username/nome/foto) de qualquer numero sem autenticacao.
  - **Reconstrucao + correcao**: `20260725060000_phone_multi_account_limit.sql` reconstroi (idempotente, so documenta — schema_migrations ja o tinha registado nessa data) o que foi aplicado no incidente, para restaurar a trilha de auditoria. `20260725093000_phone_multi_account_limit_harden.sql` corrige os tres problemas: limite exacto de **6** contas por numero; `pg_advisory_xact_lock` por numero de telefone dentro da propria transacao de signup (torna count+insert atomico); `REVOKE EXECUTE ... FROM anon` em `find_accounts_by_phone` (so `authenticated`/`service_role`).
  - **Validado ao vivo** (transaccoes com `ROLLBACK`, nada persistido): 6 contas com o mesmo numero sintetico foram criadas com sucesso; a 7a foi rejeitada com `PHONE_ACCOUNT_LIMIT_REACHED ... (limite 6)`. Grants de `find_accounts_by_phone` confirmados sem `anon`.
  - **Codigo da app** (`src/app/api/auth/signup/route.ts`): o SDK `supabase-js` descarta o corpo de erros 500 do Admin API (fica `{}`), o que escondia a distincao entre "limite de telefone atingido" e qualquer outra falha 500. Signup passou a chamar o Admin REST API via `fetch` directo (`createAuthUser`), que preserva `{ code: 'P0001', message }`, e devolve ao utilizador uma mensagem clara ("Esse numero ja tem o maximo de contas Pulse permitidas...") em vez de erro generico. Novo evento `signup_phone_limit_reached` em `src/lib/security/log.ts`.
  - `tsc --noEmit`, `next build`, `eslint` (zero erros/avisos novos vs. baseline) e a suite de 36 testes limpos apos a mudanca.
  - **Pendente**: nenhuma UI de "esta conta e uma das X ligadas a este numero" no login ainda — `find_accounts_by_phone` existe para isso mas nao e chamada por nenhum ecra. Fora de escopo desta entrega; decisao de produto/UX separada se/quando o multi-conta por numero precisar de selector visivel.

- [x] Duas lacunas residuais de `docs/UX_VOICE.md` §4 encontradas em auditoria (25/07/2026, mesma entrega acima): `comment-thread.tsx` tinha dois textos diferentes para o mesmo placeholder de comentario (`CommentComposer` prop default ja correcta vs. chamada no componente-pai a sobrepor com string antiga "Adiciona um comentario...") — removida a sobreposicao, prop default passa a vencer; estado vazio de comentarios ("Ainda sem comentarios.") e de sugestoes de pessoas ("Sem sugestoes por agora. Volta mais tarde.") nao seguiam o padrao "contexto + convite" do guia §7 — alinhados ("Diz a primeira coisa." / "Explora para encontrar pessoas novas."). So strings, sem logica alterada.
